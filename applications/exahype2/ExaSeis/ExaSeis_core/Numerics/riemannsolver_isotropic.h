...
#pragma simd
      for (int j = 0; j < basisSize; j++) {
        ...
        compute_fluctuations_left(zs_m, Tm_m, Tm_hat_m, vm_m, vm_hat_m, FLm);
        compute_fluctuations_left(zs_m, Tl_m, Tl_hat_m, vl_m, vl_hat_m, FLl);

        compute_fluctuations_right(zp_p, Tn_p, Tn_hat_p, vn_p, vn_hat_p, FRn);
        compute_fluctuations_right(zs_p, Tm_p, Tm_hat_p, vm_p, vm_hat_p, FRm);
        compute_fluctuations_right(zs_p, Tl_p, Tl_hat_p, vl_p, vl_hat_p, FRl);
  ...
  /*
    template<class Shortcuts,int basisSize, int numberOfVariables, int numberOfData, int surface = 2>
    void riemannSolver_vect(double* FL,double* FR,const double* const QL,const double* const QR,const double dt,const
    int direction, bool isBoundaryFace, int faceIndex){

      Shortcuts s;
      constexpr int num_nodes = basisSize*basisSize;

      double j_m[num_nodes][3][3];
      double j_p[num_nodes][3][3];

      kernels::idx2 idx_normal(num_nodes,3);
      double norm_p[num_nodes],norm_m[num_nodes];

      for (int n = 0; n < num_nodes; n++) {
      const double* Q_p = QR+idx_QLR(i,j,0);
      const double* Q_m = QL+idx_QLR(i,j,0);

      norm_p[n] = std::sqrt(Q_p[s.metric_derivative + 0 + direction * 3] * Q_p[s.metric_derivative + 0 + direction * 3]
    + Q_p[s.metric_derivative + 1 + direction * 3] * Q_p[s.metric_derivative + 1 + direction * 3] +
                        Q_p[s.metric_derivative + 2 + direction * 3] * Q_p[s.metric_derivative + 2 + direction * 3]);
      j_p[n][0][0] = Q_p[s.metric_derivative + 0 + direction * 3];
      j_p[n][0][1] = Q_p[s.metric_derivative + 1 + direction * 3];
      j_p[n][0][2] = Q_p[s.metric_derivative + 2 + direction * 3];

      norm_m[n] = std::sqrt(Q_m[s.metric_derivative + 0 + direction * 3] * Q_m[s.metric_derivative + 0 + direction * 3]
    + Q_m[s.metric_derivative + 1 + direction * 3] * Q_m[s.metric_derivative + 1 + direction * 3] +
                        Q_m[s.metric_derivative + 2 + direction * 3] * Q_m[s.metric_derivative + 2 + direction * 3]);

      j_m[n][0][0] = Q_m[s.metric_derivative + 0 + direction * 3];
      j_m[n][0][1] = Q_m[s.metric_derivative + 1 + direction * 3];
      j_m[n][0][2] = Q_m[s.metric_derivative + 2 + direction * 3];
      }


      //compute tractions
      double Tractions_arr[*3];
      double* Tractions[3];
      Tractions[0] = Tractions_arr;
      Tractions[1] = Tractions_arr + basisSize * basisSize;
      Tractions[2] = Tractions_arr + basisSize * basisSize * 2;
