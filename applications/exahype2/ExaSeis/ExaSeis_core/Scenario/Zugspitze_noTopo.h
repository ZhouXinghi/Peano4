#ifndef EXASEIS_SCENARIOZUGSPITZE_NO_TOPO_HEADER
#define EXASEIS_SCENARIOZUGSPITZE_NO_TOPO_HEADER

#include "Scenario.h"

template <class Shortcuts,int basisSize>
class Zugspitze_noTopo : public Scenario<Shortcuts,basisSize>{
 private:
  double m_pointSourceLocation[3];
  
 public:
 Zugspitze_noTopo(DomainInformation* info):Scenario<Shortcuts,basisSize>(info){
    //place point source in center of domain
    double center[3];
    info->getCenter(center);
    m_pointSourceLocation[0] = 4424.756;
    m_pointSourceLocation[1] =   10.000;
...
#error Whole space problem only defined for Isotropic and Anisotropic Scenario
  ...
  void setPointSourceVector(const double* const Q,
    ...
    assertion2(n == 0, "Only a single pointSource for Zugspitze_noTopo",n);
