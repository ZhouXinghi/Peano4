...
parser = argparse.ArgumentParser(
    description="ExaHyPE 2 - Acoustic Wave Application Script"
)
...
my_project = exahype2.Project(
    ...
    project_name="AcousticWave" + args.scenario,
    directory=".",
    executable=executable_name,
)

if args.solver in aderdg_rusanov_solvers:
    solver_name = "AcousticWave_ADERDG"
elif args.solver in rkdg_rusanov_solvers:
    solver_name = "AcousticWave_RKDG"
elif args.solver in fv_rusanov_solvers:
    solver_name = "AcousticWave_FV"
...
if args.dim == 2:
    my_solver.plot_description = "p, u, v"
else:
    my_solver.plot_description = "p, u, v, w"

my_solver.add_user_solver_includes(
    """
#include "AcousticWaveKernels.h"
"""
)
...
my_project.constants.define_value("WAVE_SPEED", str(wave_speed))
my_project.constants.define_value("SIGMA", str(sigma))
...
print("Regenerate all code by running 'python3 acoustic-wave.py' again")
print(
    "Generate (and convert) any postprocessing data by calling 'python3 acoustic-wave.py --visualise'"
)
