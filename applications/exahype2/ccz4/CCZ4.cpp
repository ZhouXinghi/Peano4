...
tarch::logging::Log   applications::exahype2::ccz4::CCZ4::_log( "applications::exahype2::ccz4::CCZ4" );

#ifdef IncludeTwoPunctures
//pre-process, solve the puncture equations
void applications::exahype2::ccz4::CCZ4::prepare(TP::TwoPunctures* tp){
  ...
  //following data reference: https://journals.aps.org/prd/pdf/10.1103/PhysRevD.69.024006
	if (CCZ4swi==2 and CCZ4BBHType==1){
		...
		tp->par_m_plus=0.48595; tp->par_m_minus=0.48595;
		//tp->target_M_plus=999;//adm mass
		tp->par_P_plus[0]=0.0; tp->par_P_plus[1]=0.095433; tp->par_P_plus[2]=0.0;//linear momentum
		tp->par_S_plus[0]=0.0; tp->par_S_plus[1]=0.0; tp->par_S_plus[2]=0.0;//spin
		//tp->target_M_minus=999;//adm mass
		tp->par_P_minus[0]=0.0; tp->par_P_minus[1]=-0.095433; tp->par_P_minus[2]=0.0;//linear momentum
		tp->par_S_minus[0]=0.0; tp->par_S_minus[1]=0.0; tp->par_S_minus[2]=0.0; //spin		
		tp->grid_setup_method="evaluation"; //evaluation or Taylor expansion
		tp->TP_epsilon=1e-6;}
...
applications::exahype2::ccz4::CCZ4::CCZ4() {
...
void applications::exahype2::ccz4::CCZ4::initialCondition(
...
void applications::exahype2::ccz4::CCZ4::sourceTerm(
 ...
 // for (int i=NumberOfUnknowns; i<NumberOfUnknowns+NumberOfAuxiliaryVariables; i++) {
  //  S[i] = 0.0;
  //}
  applications::exahype2::ccz4::CCZ4::sourceTerm(Q, volumeX, volumeH, t, dt, S, Offloadable::Yes);
 /*
...
void applications::exahype2::ccz4::CCZ4::boundaryConditions(
...
double applications::exahype2::ccz4::CCZ4::maxEigenvalue(
  const double * __restrict__ Q, // Q[59+0],
  const tarch::la::Vector<Dimensions,double>&  faceCentre,
  const tarch::la::Vector<Dimensions,double>&  volumeH,
  double                                       t,
...
void applications::exahype2::ccz4::CCZ4::nonconservativeProduct(
  const double * __restrict__ Q, // Q[59+0],
...
)
{
  ...
  //  BgradQ[i] = 0.0;
  //}
  ncp(BgradQ, Q, gradQSerialised, normal, CCZ4LapseType, CCZ4ds, CCZ4c, CCZ4e, CCZ4f, CCZ4bs, CCZ4sk, CCZ4xi, CCZ4mu, CCZ4SO);
}
#if defined(GPUOffloadingOMP)
#pragma omp end declare target
#endif

::exahype2::RefinementCommand applications::exahype2::ccz4::CCZ4::refinementCriterion(
  ...
  if (CCZ4ReSwi==2) { //
    if (tarch::la::equals(t,0.0)){  //static AMR, for bbh, type==1
      constexpr int NumberOfRefinementLayers = 2;
//    current "standard" refinement pattern
      double Radius[NumberOfRefinementLayers] = {8, 6};
  ...
  if (CCZ4ReSwi==4){ //5 level, not used
 ...
 //   with some alter
  ...
  if (CCZ4ReSwi==5){ //4 level, not used yet
    if (tarch::la::equals(t,0.0)){   
      constexpr int NumberOfRefinementLayers = 3;
//    current "standard" refinement pattern
...
//      double MaxH[NumberOfRefinementLayers]   = {0.3, 0.15};
  ...
  if (CCZ4ReSwi==6){
    if (tarch::la::equals(t,0.0)){  //static AMR, for bbh, type==0, 2
      constexpr int NumberOfRefinementLayers = 2;
  ...
  if (CCZ4ReSwi==7){ 
    if (tarch::la::equals(t,0.0)){  //static AMR, for sbh
      constexpr int NumberOfRefinementLayers = 2;
