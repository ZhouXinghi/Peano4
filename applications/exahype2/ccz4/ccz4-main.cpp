// **********************************************************************************************
// This file is modifed from the original version generated by exahype2.
// You are free to change the content here, but do not delete it or the 
// auto-generated main file will disable the additional step and causing compiling errors.
...
//! [Additional mesh sweep include]
#if defined(USE_ADDITIONAL_MESH_TRAVERSAL)
  #include "observers/AdditionalMeshTraversal.h"
#endif
//! [Additional mesh sweep include]

using namespace applications::exahype2::ccz4;
...
void step() {
  ...
  #if defined(USE_ADDITIONAL_MESH_TRAVERSAL)
  case repositories::StepRepository::Steps::AdditionalMeshTraversal:
    {
      tarch::logging::LogFilter::getInstance().switchProgramPhase( "additional-MeshTraversal" );
   
      repositories::suspendSolversForOneGridSweep();
      observers::AdditionalMeshTraversal observer;
      peano4::parallel::SpacetreeSet::getInstance().traverse(observer);
    }
    break;
  #endif
  ...
  const bool isPeanoComputeNode = not tarch::mpi::Rank::getInstance().isGlobalMaster();
      ...
      if (isGlobalMaster) {
        while (selectNextAlgorithmicStep()) {
          ...
          timePerMeshSwepMeasurement.setValue(watch.getCalendarTime());
          logInfo("main()", "time per mesh sweep (current/average): " << std::fixed << std::setprecision(2) << watch.getCalendarTime() << " / " << timePerMeshSwepMeasurement.getValue() );
        }
        ...
        logInfo(
          "main()",
          "plotting:                    " << plotMeasurement.getAccumulatedValue() << "s\t" << plotMeasurement.toString()
        );
        logInfo(
          "main()",
        ...
        logInfo(
          "main()",
          "average time per mesh sweep: " << timePerMeshSwepMeasurement.getValue() << "\t" << timePerMeshSwepMeasurement.toString()
        );
