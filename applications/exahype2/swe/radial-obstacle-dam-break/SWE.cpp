...
void applications::exahype2::swe::SWE::initialCondition(
  ...
  if (distanceFromOrigin >= 2 * DAM_RADIUS and distanceFromOrigin <= 3 * DAM_RADIUS) {
    Q[s::h] = 0.0; // Dry ring
  }

  Q[s::b] = distanceFromOrigin >= 2 * DAM_RADIUS
                and distanceFromOrigin <= 3 * DAM_RADIUS
              ? BATHYMETRY_INSIDE_OBSTACLE
              : BATHYMETRY_OUTSIDE_OBSTACLE;
}
