...
class FixedSubcyclingTimeSteppingCodeSnippets( SolverCodeSnippets ):
  """
  ...
  def __init__(self, remove_accumulation_errors):
    self._remove_accumulation_errors = remove_accumulation_errors


  def create_abstract_solver_user_declarations(self):
    return """
private:
  double _finestMeshTimeStepSize;
public:
...
double {{FULL_QUALIFIED_NAMESPACE}}::{{CLASSNAME}}::getAdmissibleTimeStepSize() const {
  ...
  def create_compute_time_step_size(self):
    if self._remove_accumulation_errors:
      return """
