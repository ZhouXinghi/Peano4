...
class ConfigurationParametersModel(AbstractModelBaseClass):
    def generateCode(self):
        ...
        if self.context["kernelType"] == "aderdg":
            self.context["isLinearCText"] = (
                "true" if self.context["isLinear"] else "false"
            )  # c++ true/false instead of True/False
            ...
            self.context["usePointSourcesFactor"] = (
                1 if self.context["usePointSources"] else 0
            )
            self.context["useMaterialParamFactor"] = (
                1 if self.context["useMaterialParam"] else 0
            )
            self.context["useSourceOrNCPFactor"] = (
                1 if self.context["useSourceOrNCP"] else 0
            )
            ...
            if self.context["isLinear"]:
                if self.context["useSplitCK"]:
                    ...
                    self.context["lFiSize"] = (
                        nDim * nDof * (nDof**nDim) * nVarPad
                    )  # nVarPad*(nDof**(nDim+1))*(2*nDim+1) # Todo JMG see if 2dim+1 or 2dim
                ...
                if self.context["predictorRecompute"]:
                    if self.context["useVectPDE"]:
                        ...
                        if self.context["useFlux"]:
                            self.context["lFhiSize"] = (
                                nDofPad * nVar * nDof * nDof3D * nDim
                            )
        ...
        self.render(
            (self.context["kernelType"], "configurationParameters_cpph.template"),
            "ConfigurationParameters.cpph",
        )
