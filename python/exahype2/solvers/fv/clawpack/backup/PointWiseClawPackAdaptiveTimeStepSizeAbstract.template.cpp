...
void {{NAMESPACE | join("::")}}::{{CLASSNAME}}::finishTimeStep() {
  ...
  if ( std::isnan(_timeStepSize) or std::isinf(_timeStepSize) ) {
    ::tarch::triggerNonCriticalAssertion( __FILE__, __LINE__, "_timeStepSize>0", "invalid (NaN of inf) time step size: " + std::to_string(_timeStepSize) );
  }
  if (tarch::la::smallerEquals(_timeStepSize,0.0,1e-10) ) {
    ::tarch::triggerNonCriticalAssertion( __FILE__, __LINE__, "_timeStepSize>0", "degenerated time step size of " + std::to_string(_timeStepSize) );
  }
