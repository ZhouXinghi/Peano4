...
"""!
Generates 2D random particle distribution.
"""

fileName = "test_sml_2D.hdf5"
boxSize = 1.0
...
# Build the arrays
coords = np.zeros((npart**2, 3))
v = np.zeros((npart**2, 3))
ids = np.linspace(1, npart**2, npart**2)
m = np.ones(npart**2)
dx = boxSize / npart
h = np.ones(npart**2) * 1.2 * dx
u = np.ones(npart**2)
...
displacement = (2.0 * rng.random(coords.shape) - 1.0) * 0.199 * dx
